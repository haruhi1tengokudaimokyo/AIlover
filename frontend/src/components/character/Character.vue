...
async function handleRemoveCharacter() {
  try {
    ...
    if (res.data.result ==='success') {
      emits('remove',props.character.id)
    }
  } catch(err) {
  }
}
...
<template>
  <div>
    <div class="avatar cursor-pointer" @mouseover="isHover=true" @mouseout="isHover=false" @click="openChatField">
      <div class="w-60 h-100 rounded2xl relative">
    ...
    <RouterLink :to="{name:'user-space-index',params:{user_id:character.author.user_id}}" class="flex items-center mt-4 gap-2">
      <div class="avatar">
